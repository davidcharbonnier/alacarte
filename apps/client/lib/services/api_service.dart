...
abstract class ApiService {
  ...
  static Future<void> _processConnectivityChange(
    List<ConnectivityResult> results,
  ) async {
  ...
  static Future<void> _checkApiReachability() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _debugLog(
          '🟠 API returned non-200 status: ${response.statusCode} - server offline',
        );
    ...
    if (_state != newState) {
      ...
      switch (newState) {
        case ConnectivityState.online:
          _debugLog('🟢 🌐 Connected to À la carte - app fully functional');
          break;
  ...
  Future<ApiResponse<Map<String, dynamic>>> googleOAuthExchange(
    String idToken,
    String accessToken,
  ) async {
    return handleResponse<Map<String, dynamic>>(
      post(
        ApiConfig.googleOAuth,
  ...
  Future<ApiResponse<Map<String, dynamic>>> completeProfile(
    ...
    return handleResponse<Map<String, dynamic>>(
      post(
        ApiConfig.profileComplete,
        data: {'display_name': displayName, 'discoverable': discoverable},
      ),
      (data) => data as Map<String, dynamic>,
    );
  }

  Future<ApiResponse<Map<String, dynamic>>> checkDisplayNameAvailability(
    String displayName,
  ) async {
    return handleResponse<Map<String, dynamic>>(
      get(
        ApiConfig.profileCheckDisplayName,
  ...
  Future<ApiResponse<Map<String, dynamic>>> getCommunityStats(
    ...
    if (_communityStatsCache.containsKey(cacheKey) &&
        _communityStatsCacheTime.containsKey(cacheKey)) {
      final age = DateTime.now().difference(
        _communityStatsCacheTime[cacheKey]!,
      );
  ...
  Future<ApiResponse<User>> updateDisplayName(String displayName) async {
    return handleResponse<User>(
      _dio.patch(ApiConfig.userMePatch, data: {'display_name': displayName}),
      (data) {
  ...
  Future<ApiResponse<User>> updateDiscoverable(bool discoverable) async {
    return handleResponse<User>(
      _dio.patch(ApiConfig.userMePatch, data: {'discoverable': discoverable}),
      (data) {
        final userData = data['user'] ?? data;
        return User.fromJson(userData as Map<String, dynamic>);
      },
  ...
  Future<ApiResponse<bool>> deleteAccount() async {
    return handleEmptyResponse(delete(ApiConfig.userMeDelete));
  }
  ...
  Future<ApiResponse<Map<String, dynamic>>> bulkRemoveUserFromShares(
    int userId,
  ) async {
    return handleResponse<Map<String, dynamic>>(
