# admin-spa-auth Specification

## Purpose

Google OAuth authentication for the admin SPA: direct browser-based sign-in via `@react-oauth/google`, JWT-based session management in `sessionStorage`, admin privilege verification, automatic JWT attachment to API requests via Axios interceptors, token expiry handling, route protection with TanStack Router, and sign-out.
