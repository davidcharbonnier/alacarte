...
- [x] 18.1 Add `ref.read(schemaProvider.notifier).refreshSchema(widget.itemType)` to all pull-to-refresh callbacks in `item_type_screen.dart`:
  - _buildAllItemsTab
...
- [x] 18.2 Manual test: modify schema in admin, pull-to-refresh on item list, verify schema updates without returning to home screen

## 19. Personal Items Data Source

### Backend (API)

- [x] 19.1 Add `Rated bool` field to `QueryParams` struct in `services/query_builder.go`
- [x] 19.2 Add `RatedByUserID int` field to `QueryParams` struct (set by controller from JWT when `Rated` is true)
- [x] 19.3 Implement `rated` subquery filter in `BuildListQuery`: when `Rated` is true, filter items to those where user has a rating (author OR viewer via rating_viewers join), excluding soft-deleted ratings
- [x] 19.4 Parse `?rated=true` in `DynamicItemList` controller, extract user ID from JWT, set `params.Rated = true` and `params.RatedByUserID = <jwt_user_id>` — silently ignore if unauthenticated
- [x] 19.5 Write `TestEAVQueryBuilder_RatedBy` in `services/query_builder_test.go`: create items with/without ratings, verify `Rated=true` returns only rated, verify viewer-shared items included, verify pagination within subset
- [x] 19.6 Write `TestDynamicItemList_RatedBy` in `controllers/dynamic_item_controller_test.go`: integration test via httptest, verify auth required, verify correct items returned

### Client Service

- [x] 19.7 Add optional `bool rated = false` parameter to `DynamicItemService.getItemsByType()`, append `rated=true` to query params when set

### Client Provider

- [x] 19.8 Add user-rated state fields to `DynamicItemState`: `userRatedItemsByType`, `userRatedLoadingByType`, `userRatedCurrentPageByType`, `userRatedTotalPagesByType`, `userRatedTotalByType`, `userRatedIsLoadingMoreByType` with defaults, copyWith, and accessors
- [x] 19.9 Add `loadUserRatedItems(String type)` to `DynamicItemNotifier`: call `getItemsByType(type, rated: true, page: 1)`, store in user-rated maps
- [x] 19.10 Add `loadMoreUserRatedItems(String type)` to `DynamicItemNotifier`: fetch next page, append, guard against double-load/no-more
- [x] 19.11 Add `refreshUserRatedItems(String type)` to `DynamicItemNotifier`: force-refresh user-rated items from page 1
- [x] 19.12 Add user-rated convenience methods to `ItemProviderHelper`: `getUserRatedItems`, `userRatedHasMore`, `userRatedIsLoadingMore`, `userRatedTotalItems`, `loadUserRatedItems`, `loadMoreUserRatedItems`, `refreshUserRatedItems`
- [x] 19.13 Add user-rated family providers: `userRatedItemsForTypeProvider`, `userRatedHasMoreProvider`, `userRatedIsLoadingMoreProvider`

### Client UI

- [x] 19.14 Add separate `ScrollController _myListScrollController` to `_ItemTypeScreenState`, attach scroll listener in initState, dispose in dispose
- [x] 19.15 Add `_onMyListScroll()` listener triggering `ItemProviderHelper.loadMoreUserRatedItems(ref, widget.itemType)` at 200px threshold
- [x] 19.16 Update `_buildMyListTab` to read from `userRatedItems` instead of `itemsByType`
- [x] 19.17 Replace plain `ListView` in My List with `ListView.builder` using `_myListScrollController` and load-more indicator
- [x] 19.18 Trigger `loadUserRatedItems` on My List tab activation (tab change listener or initState)
