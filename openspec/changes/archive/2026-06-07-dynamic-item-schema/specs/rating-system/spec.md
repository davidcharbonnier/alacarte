# rating-system Specification (Delta)
...
### Requirement: Item Type Field on Ratings

**Rationale:** After all items are unified in the `items` table, `item_id` alone uniquely identifies what a rating is about. The `item_type` column is redundant. Item type information can be derived by joining through `items.schema_id → item_type_schemas` when needed.

- The `item_type` column SHALL be removed from the `ratings` table
- The `item_type` field SHALL be removed from all rating API request/response bodies
- The `?type=` query parameter SHALL be removed from `RatingByAuthor` and `RatingByViewer`
- The `:type` URL path parameter SHALL be removed from `RatingByItem` and `GetCommunityStats`
